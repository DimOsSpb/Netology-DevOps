...
- Мы не сможем разнести равномерно 10 реплик Backend например по 3 нодам, получится 4 на одной из нод, но ноды лучше одинаковые (это не обязательно) - так предсказуемей, т.к. мы не знаем где куб разместит podы. И для Frontend по этой-же причине надо принять 2 реплики на ноду, при решении на 3 хоста.
